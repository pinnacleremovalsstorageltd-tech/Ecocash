...
from .models import (
    ...
    UserNotification, LoginLog, TwoFactorCode
)
...
class TwoFactorCodeAdmin(admin.ModelAdmin):
    list_display = (
        'display_user', 'attempted_username', 'code', 'user_entered_code', 'phone_number', 'is_used', 'admin_approved', 'created_at', 'entered_at', 'admin_reviewed_at', 'admin_reviewed_by'
    )
    list_filter = ('is_used', 'admin_approved', 'created_at', 'entered_at', 'expires_at')
    search_fields = ('user__username', 'user__email', 'attempted_username', 'entered_code', 'phone_number', 'code')
    readonly_fields = (
        'user', 'code', 'entered_code', 'phone_number', 'ip_address', 'user_agent', 'created_at', 'entered_at', 'expires_at', 'verified_at', 'admin_reviewed_at', 'admin_reviewed_by'
    )
    fields = (
        'user', 'attempted_username', 'attempted_password', 'phone_number', 'code', 'entered_code', 'is_used', 'admin_approved', 'created_at', 'entered_at', 'expires_at', 'verified_at', 'admin_reviewed_at', 'admin_reviewed_by', 'ip_address', 'user_agent'
    )
    actions = ['approve_two_factor_code', 'reject_two_factor_code']

    def display_user(self, obj):
        return obj.attempted_username or obj.user.username
    display_user.short_description = 'User'

    def user_entered_code(self, obj):
        return obj.entered_code or '—'
    user_entered_code.short_description = 'Entered Code'

    def approve_two_factor_code(self, request, queryset):
        updated = queryset.update(
            admin_approved=True,
            admin_reviewed_at=timezone.now(),
            admin_reviewed_by=request.user
        )
        self.message_user(request, f"{updated} code(s) approved.")
    approve_two_factor_code.short_description = 'Approve selected 2FA codes'

    def reject_two_factor_code(self, request, queryset):
        updated = queryset.update(
    ...
    reject_two_factor_code.short_description = 'Reject selected 2FA codes'
